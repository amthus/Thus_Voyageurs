...
@section('title', 'Ajouter un Séjour')
...
        </div>
        <div class="card-body">
            <form action="{{ route('sejours.store') }}" method="POST">
                @csrf
                <div class="mb-3">
...
                            <option value="{{ $voyageur->id_voyageur }}" {{ old('id_voyageur') == $voyageur->id_voyageur ? 'selected' : '' }}>{{ $voyageur->nom }} {{ $voyageur->prenom }}</option>
                        @endforeach
                    </select>
                    @error('id_voyageur')
...
                    <input type="date" class="form-control @error('debut') is-invalid @enderror" id="debut" name="debut" value="{{ old('debut') }}" required>
                    @error('debut')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
...
                    <input type="date" class="form-control @error('fin') is-invalid @enderror" id="fin" name="fin" value="{{ old('fin') }}" required>
                    @error('fin')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
